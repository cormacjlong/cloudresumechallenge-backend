...
resource "null_resource" "apim_customdomain" {
  ...
  provisioner "local-exec" {
    command = "az login && az apim update -n ${self.triggers.apim_name} -g ${self.triggers.rg} --set hostnameConfigurations='[{\"hostName\":\"${self.triggers.api_url}\",\"type\":\"Proxy\",\"certificateSource\":\"Managed\"}]'"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "az login && az apim update -n ${self.triggers.apim_name} -g ${self.triggers.rg} --remove hostnameConfigurations"
  }
}
